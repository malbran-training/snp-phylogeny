...
\PY{n+nb}{cd}\PY{+w}{ }\PYZti{}/course\PYZus{}data/snp\PYZhy{}phylogeny/data
\end{Verbatim}
\end{tcolorbox}
...
Phandango & https://jameshadfield.github.io/phandango/\#/ & 1.3.0 \\
\bottomrule
\end{longtable}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{n+nb}{cd}\PY{+w}{ }\PYZti{}/course\PYZus{}data/snp\PYZhy{}phylogeny/data/typhi
\end{Verbatim}
\end{tcolorbox}
...
The first step once you have obtained your sequence data (FASTQ) from
the sequencing machine is to QC the data. After QC, the sequence data is
matched or aligned to a reference genome (FASTA) in a process called
read mapping to produce a set of read aligments (SAM/BAM). These read
alignments are inspected to identify differences between the aligned
reads and the reference genome. This process is called variant calling
and produces VCF files. In fact during this process we capture
information about every position in the genome (variant and non-variant
sites) in the VCFs. Each site in the VCF has a set of quality filters
applied and any sites identified as low quaility (e.g.~less than 4 reads
aligned at that position) are marked as low quality in the VCF to
produce a filtered VCF. We use this filtered VCF file in a process
called consensus caling to reconstruct a consensus \textit{pseudosequence}
or \textit{pseudogenome} for our sample (FASTA). In the
\textit{pseudogenome}, any sites marked as low quality will be represented
as an N in the reconstructed sequence. These pseudogenomes (multi-FASTA)
are then aligned and variation identified and used to reconstruct a
phylogeny of our samples.
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
fastqc\PY{+w}{ }\PYZhy{}o\PY{+w}{ }qc\PYZus{}results\PY{+w}{ }*.fastqc.gz
\end{Verbatim}
\end{tcolorbox}
...
\begin{center}
\includegraphics[MultiQC results]{img/multiqc.png}
\end{center}
...
\item
  Which sample has the largets yield (mose sequence data)?
\end{enumerate}
...
ERR5243693\PYZus{}1.trim.fastq.gz\PY{+w}{ }ERR5243693\PYZus{}2.trim.fastq.gz\PY{+w}{ }\PYZgt{}\PY{+w}{ }\PY{l+s+se}{\PYZbs{}}
ERR5243693.sam
\end{Verbatim}
\end{tcolorbox}

    This may take a few minutes to run. When complete, convert the sam file
to a bam file with samtools:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
samtools\PY{+w}{ }view\PY{+w}{ }\PYZhy{}@\PY{+w}{ }\PY{l+m}{2}\PY{+w}{ }\PYZhy{}bhS\PY{+w}{ }\PYZhy{}o\PY{+w}{ }ERR5243693.bam\PY{+w}{ }ERR5243693.sam
\end{Verbatim}
\end{tcolorbox}
...
aligned at that position and make a call about what the base is in the
sample at that position. This information will be stored in the a VCF
file and if there are any differences then this will be marked as a
variant (snp/indel) in this VCF file.
...
INFO/ADF,INFO/ADR\PY{+w}{ }ERR5243693.sorted.bam\PY{+w}{ }\PY{p}{|}\PY{+w}{ }bcftools\PY{+w}{ }call\PY{+w}{ }\PY{l+s+se}{\PYZbs{}}
...
\end{tcolorbox}
    ...
    Again this may take some time to run. When it completes, index the VCF
file:
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
bcftools\PY{+w}{ }view\PY{+w}{ }ERR5243693.vcf.gz\PY{+w}{ }\PY{p}{|}\PY{+w}{ }less
\end{Verbatim}
\end{tcolorbox}

    Notice that the VCF has information about every site in the genome (not
just variants).
...
\PYZhy{}\PYZhy{}soft\PYZhy{}filter\PY{+w}{ }LowQual\PY{+w}{ }\PYZhy{}\PYZhy{}exclude\PY{+w}{ }\PY{l+s+s2}{\PYZdq{}QUAL\PYZlt{}25 || FORMAT/DP\PYZlt{}10 \PYZbs{}}
\PY{l+s+s2}{|| MAX(FORMAT/ADF)\PYZlt{}2 || MAX(FORMAT/ADR)\PYZlt{}2 || \PYZbs{}}
...
\end{tcolorbox}
    ...
    The filtering step may take some time ti run. When complete, index the
filtered VCF file:
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
bcftools\PY{+w}{ }view\PY{+w}{ }ERR5243693.filtered.vcf.gz\PY{+w}{ }\PY{p}{|}\PY{+w}{ }less
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
vcf2pseudogenome.py\PY{+w}{ }\PYZhy{}r\PY{+w}{ }../ref/Styphi\PYZus{}CT18.fa\PY{+w}{ }\PYZhy{}b\PY{+w}{ }ERR5243693.filtered.vcf.gz\PY{+w}{ }\PYZhy{}o\PY{+w}{ }ERR5243693.fas
\end{Verbatim}
\end{tcolorbox}
...
Remember to reconstruct the phylogeny of samples we need a multi fasta
alignment of our sequences.
...
being the same length we do not have to perform a mult sequence
alignment.
...
Sometimes you may also want to include the reference genome used in the
mapping step in your tree. To do this add the reference sequence to the
multifasta alignment of your samples.

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
cat\PY{+w}{ }../ref/Styphi\PYZus{}CT18.fa\PY{+w}{ }aligned\PYZus{}pseudogenomes.aln\PY{+w}{ }\PYZgt{}\PY{+w}{ }ref\PYZus{}and\PYZus{}aligned\PYZus{}pseudogenomes.aln
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
snp\PYZhy{}sites\PY{+w}{ }\PYZhy{}o\PY{+w}{ }ref\PYZus{}and\PYZus{}aligned\PYZus{}pseudogenomes.snps.aln\PY{+w}{ }ref\PYZus{}and\PYZus{}aligned\PYZus{}pseudogenomes.aln
\end{Verbatim}
\end{tcolorbox}

    We can see how many invariant sites were removed (and what proportion of
A, T, G, C they were) using

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
snp\PYZhy{}sites\PY{+w}{ }\PYZhy{}C\PY{+w}{ }ref\PYZus{}and\PYZus{}aligned\PYZus{}pseudogenomes.aln
\end{Verbatim}
\end{tcolorbox}
...
\end{tcolorbox}
    ...
    Draw the tree with IQ\_TREE:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
iqtree\PY{+w}{ }\PY{l+s+se}{\PYZbs{}}
\PY{+w}{    }\PYZhy{}fconst\PY{+w}{ }\PY{k}{\PYZdl{}(}\PY{+w}{ }snp\PYZhy{}sites\PY{+w}{ }\PYZhy{}C\PY{+w}{ }ref\PYZus{}and\PYZus{}aligned\PYZus{}pseudogenomes.aln\PY{+w}{ }\PY{k}{)}\PY{+w}{ }\PY{l+s+se}{\PYZbs{}}
\PY{+w}{    }\PYZhy{}alrt\PY{+w}{ }\PY{l+m}{1000}\PY{+w}{ }\PYZhy{}B\PY{+w}{ }\PY{l+m}{1000}\PY{+w}{ }\PYZhy{}m\PY{+w}{ }MFP\PY{+w}{ }\PYZhy{}czb\PY{+w}{ }\PY{l+s+se}{\PYZbs{}}
\PY{+w}{    }\PYZhy{}s\PY{+w}{ }pseudogenomes.snpsites.aln\PY{+w}{ }\PY{l+s+se}{\PYZbs{}}
\PY{+w}{    }\PYZhy{}nt\PY{+w}{ }AUTO\PY{+w}{ }\PY{l+s+se}{\PYZbs{}}
\PY{+w}{    }\PYZhy{}ntmax\PY{+w}{ }\PY{l+m}{2}\PY{+w}{ }\PY{l+s+se}{\PYZbs{}}
...
specify the multiple sequence alignment using -s clean.full.SNPs.aln ask
IQ-TREE to take account of missing invariant sites using -fconst
\$(snp-sites -C clean.full.aln) specify an evolutionary model we want
IQ-TREE to use -m GTR+F+I tell IQ-TREE to use a maximum of 2 CPUs
(threads) and 2GB memory -T 2 -mem 2G perform 1000 ultrafast bootstraps
-B 1000 use sample M66 as an outgroup -o M66

    Our maximum likelihood tree is labelled XXX.treefile. The treefile
suffix is not always correctly identified by many tools, so we'll
relabel this as something else:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
cp\PY{+w}{ }clean.full.SNPs.aln.treefile\PY{+w}{ }clean.full.SNPs.aln.tre
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
cat\PY{+w}{ }clean.full.SNPs.aln.tre
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
figtree\PY{+w}{ }clean.full.SNPs.aln.tre\PY{+w}{ }\PY{p}{\PYZam{}}
\end{Verbatim}
\end{tcolorbox}
...
closely related to our sequences of interest but different enough to
\ldots. Examples\ldots.

    Side Note: Often you may want to incorporate a `finished reference
genome' into your tree e.g.~to use as an outgroup. (Explain what an
outgroup is and why you may want to use it) or to see how your
samples/isolates relate to them. If there is no sequence data available
for the isolate e.g.~there is only a complete reference genome then one
approach you can take is to shred the reference genome (fasta file) and
make simulated reads (remember it is a haploid genome) and treat the
simulated data as another sample and process it like the other samples.
You will then have a `pseudogenome' of the sample that will be included
in the ref\_and\_aligned\_pseudogenomes.fas file at this stage. The
practical aspect of this is beyond the scope of this tutorial. The
article below lists some popular tools for simulating reads:
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
rm\PY{+w}{ }*.trim.fastq.gz
rm\PY{+w}{ }*.sub.fastq.gz
Rm\PY{+w}{ }*.html
rm\PY{+w}{ }*.sam
rm\PY{+w}{ }ERR5243693.bam*
rm\PY{+w}{ }ERR5243695.bam*
rm\PY{+w}{ }ERR5243699.bam*
rm\PY{+w}{ }ERR5243693.vcf.gz*
rm\PY{+w}{ }ERR5243695.vcf.gz*
rm\PY{+w}{ }ERR5243699.vcf.gz*
\end{Verbatim}
\end{tcolorbox}

...
../fastq/ERR5243693\PYZus{}1.trim.fastq.gz\PY{+w}{ }../fastq/ERR5243693\PYZus{}2.trim.fastq.gz\PY{+w}{ }\PYZgt{}\PY{+w}{ }\PY{l+s+se}{\PYZbs{}}
ERR5243693.sam
\end{Verbatim}
\end{tcolorbox}
...
\item
  ../ref/Styphi\_CT18.fa is the reference to match the reads against
\item
  ../fastq/ERR5243693\_1.trim.fastq.gz and
  ../fastq/ERR5243693\_2.trim.fastq.gz are the fastq files containing
  our sequence reads (after trimming) for sample ERR5243693
\item
...
\tightlist
\item
  --fasta-ref ../ref/Styphi\_CT18.fa is the reference that the reads
  were mapped to
\item
  --min-BQ 20
\item
  --annotate
  FORMAT/AD,FORMAT/ADF,FORMAT/ADR,FORMAT/DP,FORMAT/SP,INFO/AD,INFO/ADF,INFO/ADR
...
\tightlist
\item
  --output-type v telss the program to output VCF format
\item
  --ploidy 1 tells the program that we are daling with haploid datasets
\item
  --multiallelic-caller tells the program which variant calling
  algorithm to use
\end{itemize}
...
\tightlist
\item
  --output-file ERR5243693.vcf.gz tells the program the name of the
  output file
\item
  --output-type z tells the program to create a gzipped VCF file as
  output
\end{itemize}
...
\tightlist
\item
  For sample ERR5243693 what is the reference base at position?
\item
  For sample ERR5243693 what is the allele for the sample at position?
  Is it the same or different that the reference?
\item
  For sample ERR5243693 what is the allele for the sample at position?
  Is it the same or different that the reference?
\end{enumerate}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
Now\PY{+w}{ }generate\PY{+w}{ }some\PY{+w}{ }statistics\PY{+w}{ }about\PY{+w}{ }the\PY{+w}{ }VCF\PY{+w}{ }file:
\end{Verbatim}
\end{tcolorbox}
...
\item
  --soft-filter tells the program to keep any filtered position in the
  file and mark them as \texttt{LowQual} rather than remove them
  completely from the file (hard filter)
\item
  --exclude lists the filtering criteria to apply to each position
\item
  --output-type specifies the type of output file to create, in this
  case z means a xompressed VCF file
\end{itemize}
...
Looking at the filtered VCF for sample ERR5243693: 1. Does position X
pass or fail? 2. Does position X pass or fail? 3. What is the reason for
the failure at position X?
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
vcf2pseudogenome.py\PY{+w}{ }\PYZhy{}r\PY{+w}{ }../ref/Styphi\PYZus{}CT18.fa\PY{+w}{ }\PYZhy{}b\PY{+w}{ }../variants/ERR5243693.filtered.vcf.gz\PY{+w}{ }\PYZhy{}o\PY{+w}{ }ERR5243693.fas
\end{Verbatim}
\end{tcolorbox}
...
Remember to reconstruct the phylogeny of our samples we need a
multi-fasta alignment of our sequences. We are going to use a
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
snp\PYZhy{}sites\PY{+w}{ }\PYZhy{}o\PY{+w}{ }pseudogenomes.snps.aln\PY{+w}{ }../pseudogenomes/ref\PYZus{}and\PYZus{}aligned\PYZus{}pseudogenomes.aln
\end{Verbatim}
\end{tcolorbox}
...
\PY{+w}{    }\PYZhy{}m\PY{+w}{ }GTR+G+T\PY{+w}{ }\PY{l+s+se}{\PYZbs{}}
...
\begin{itemize}
\item
  -s to specify the multiple sequence alignment file
  \texttt{pseudogenomes.snpsites.aln}
\item
  \begin{itemize}
  \tightlist
  \item
    fconst to ask IQ-TREE to take account of missing invariant sites
    \$(snp-sites -C ref\_and\_aligned\_pseudogenomes.aln calculates the
    specific values to pass to iqtree)
  \end{itemize}
\item
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
cp\PY{+w}{ }pseudogenomes.snpsites.aln.treefile\PY{+w}{ }pseudogenomes.snpsites.aln.tre
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
cat\PY{+w}{ }pseudogenomes.snpsites.aln.tre
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
figtree\PY{+w}{ }pseudogenomes.snpsites.aln.tre\PY{+w}{ }\PY{p}{\PYZam{}}
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
Run\PY{+w}{ }gubbins\PY{+w}{ }on\PY{+w}{ }your\PY{+w}{ }data:
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
run\PYZus{}gubbins.py\PY{+w}{ }\PYZhy{}c\PY{+w}{ }\PY{l+m}{2}\PY{+w}{ }\PYZhy{}p\PY{+w}{ }gubbins\PY{+w}{ }../pseudogenomes/ref\PYZus{}and\PYZus{}aligned\PYZus{}pseudogenomes.aln
\end{Verbatim}
\end{tcolorbox}
...
NB. If gubbins takes more than 10 mins to complete, we have already run
it for you - the files are available at:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
ls\PY{+w}{ }\PYZhy{}l\PY{+w}{ }\PYZti{}/course\PYZus{}data/snp\PYZhy{}phylogeny/data/gubbins\PYZus{}backup
\end{Verbatim}
\end{tcolorbox}
...
the outgroup and \texttt{shred} it to similate sequencing reads for the
...
\end{tcolorbox}
    ...
    Now let's remove any files that do not need to be kept long term:
